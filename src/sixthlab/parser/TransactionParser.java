...
public class TransactionParser {
  ...
  public static Transaction parse(String transaction, List<Account> accounts, boolean isSynchronized, PrintWriter out) {
    if (transaction.contains("from:") && transaction.contains("to:") && transaction.contains("amount:")) {
      ...
      if (isSynchronized)
        return new TransactionSynchronized(id++, amount, accounts.get(from - 1), accounts.get(to - 1), out);
      else
        return new TransactionConcurrent(id++, amount, accounts.get(from - 1), accounts.get(to - 1), out);
    } else {
